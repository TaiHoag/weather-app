...
def get_sky_color(hour):
    ...
    if 6 <= hour < 18:
        ...
        return "orange"  # Sunset color
    else:
        return "darkblue"  # Nighttime color
...
def plot_in_new_window(data, title, xlabel, ylabel):
    ...
    ax.plot(data['date'], data[ylabel], marker='o', linestyle='-', color=plot_color)
...
def visualize_hourly_weather(hourly_dataframe, canvas):
    ...
    fig.patch.set_facecolor(bg_color)
    axs[0, 0].plot(hourly_dataframe['date'], hourly_dataframe['temperature_2m'], marker='o', linestyle='-', color=plot_color)
    ...
    axs[0, 1].plot(hourly_dataframe['date'], hourly_dataframe['relative_humidity_2m'], marker='o', linestyle='-', color=plot_color)
    ...
    axs[1, 0].plot(hourly_dataframe['date'], hourly_dataframe['precipitation'], marker='o', linestyle='-', color=plot_color)
    ...
    axs[1, 1].plot(hourly_dataframe['date'], hourly_dataframe['rain'], marker='o', linestyle='-', color=plot_color)
    ...
    axs[2, 0].plot(hourly_dataframe['date'], hourly_dataframe['cloud_cover'], marker='o', linestyle='-', color=plot_color)
...
def create_gui(fetch_weather_callback):
    ...
    weather_text = tk.CTkTextbox(root, font=myfont, width=200, height=150, scrollbar_button_color="orange", corner_radius=16, border_color="orange", border_width=2)
    weather_text.grid(row=1, column=1, columnspan=2, padx=5, pady=5, sticky="nsew")
...
def main():
    ...
    root, city_entry, weather_text, canvas_frame = create_gui(lambda: display_weather_info(city_entry, weather_text, canvas_frame))
    root.mainloop()
